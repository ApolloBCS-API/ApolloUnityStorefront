...
public class CartManager : MonoBehaviour {
	...
	public static CartManager Instance {
		get {
			if (_instance == null) {
				GameObject dtMan = GameObject.Find ("Apollo_Controller") as GameObject;
				if (dtMan == null) {
					dtMan = GameObject.Instantiate (Resources.Load ("Prefabs/Apollo_Controller")) as GameObject;
				}
	...
	public void ProgessPurchase() {
		int livesToAdd = 0;
		foreach(var item in _listVirtual){
            livesToAdd += (item.Value.livesValue + item.Value.bonusValue) * item.Value.quantity;
		}
		FakeAccountController.Instance.IncreaseLives(livesToAdd);
